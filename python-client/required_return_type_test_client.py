...
class RequiredReturnTypeTestClient:
	...
	async def get_async(self):
		relative_url = f"async-task"
		params = {}
		response = await self._client.get(relative_url, params = params)
		response.raise_for_status()
	
	async def get_action_result(self):
		relative_url = f"action-result"
		params = {}
		response = await self._client.get(relative_url, params = params)
		response.raise_for_status()
	
	async def get_async_action_result(self):
		relative_url = f"async-action-result"
		params = {}
		response = await self._client.get(relative_url, params = params)
		response.raise_for_status()
	...
	async def get_my_dto_array(self) -> list[MyDto]:
		relative_url = f"array-return-type"
		params = {}
		response = await self._client.get(relative_url, params = params)
		response.raise_for_status()
	
	async def get_my_dto_collection(self) -> list[MyDto]:
		relative_url = f"collection-return-type"
		params = {}
		response = await self._client.get(relative_url, params = params)
		response.raise_for_status()
	
	async def get_my_dto_collection_async(self) -> list[MyDto]:
		relative_url = f"async-collection-return-type"
		params = {}
		response = await self._client.get(relative_url, params = params)
		response.raise_for_status()
