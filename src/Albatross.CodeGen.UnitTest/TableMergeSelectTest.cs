	...
	public class TableMergeSelectTest {
		...
		static IEnumerable<TestCaseData> MergeSelectTestCase() {
			return new TestCaseData[] {
				...
				new TestCaseData(ContactTable.Table, new SqlCodeGenOption{ Expressions={ {"@created", "getdate()" }, { "@modified", "getdate()" }, { "@createdBy", "@user" }, { "@modifiedBy", "@user" }, } }){ ExpectedResult="using (\r\n\tselect\r\n\t\t@contactID as [ContactID],\r\n\t\t@domain as [Domain],\r\n\t\t@login as [Login],\r\n\t\t@firstName as [FirstName],\r\n\t\t@lastName as [LastName],\r\n\t\t@middleName as [MiddleName],\r\n\t\t@gender as [Gender],\r\n\t\t@cell as [Cell],\r\n\t\t@address as [Address],\r\n\t\tgetdate() as [Created],\r\n\t\t@user as [CreatedBy],\r\n\t\tgetdate() as [Modified],\r\n\t\t@user as [ModifiedBy]\r\n) as src" },
			};
		}
