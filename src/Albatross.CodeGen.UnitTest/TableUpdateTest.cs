	...
	public class TableUpdateTest {
		static IEnumerable<TestCaseData> GetTestCases() {
			return new TestCaseData[] {
				...
				new TestCaseData(ContactTable.Table, new SqlCodeGenOption{ ExcludePrimaryKey = true, Variables={ { "@user", "varchar(100)"} }, Expressions={ {"created", "getdate()" }, { "createdBy", "@user"}, { "modified","getdate()"}, { "modifiedBy", "@user"} } }){ ExpectedResult = "update [test].[Contact] set\r\n\t[FirstName] = @firstName,\r\n\t[LastName] = @lastName,\r\n\t[MiddleName] = @middleName,\r\n\t[Gender] = @gender,\r\n\t[Cell] = @cell,\r\n\t[Address] = @address,\r\n\t[Created] = getdate(),\r\n\t[CreatedBy] = @user,\r\n\t[Modified] = getdate(),\r\n\t[ModifiedBy] = @user"},
			};
		}
